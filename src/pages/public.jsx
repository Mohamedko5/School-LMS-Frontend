...
export function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950" dir={dir}>
      <header className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-5 sm:py-5">
        ...
        <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={toggleLanguage}>{t("switchLanguage")}</Button><Link to="/login"><Button>{t("demoLoginButton")}</Button></Link></div>
      </header>
      <main>
        <section className="mx-auto grid min-h-[70vh] max-w-7xl grid-cols-1 items-center gap-8 px-4 py-8 sm:px-5 sm:py-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.9fr)]">
          <div className="min-w-0">
            <Badge tone="teal">{t("landingBadge")}</Badge>
            <h1 className="mt-5 max-w-3xl break-words text-4xl font-black tracking-normal text-slate-950 dark:text-white sm:text-5xl lg:text-6xl">EduBridge School LMS</h1>
          ...
          <div className="relative min-w-0">
            ...
            <Card className="relative overflow-hidden">
              <div className="mb-5 flex flex-wrap items-center justify-between gap-2"><b>Student Dashboard Preview</b><Badge>Grade 8A</Badge></div>
              ...
              <div className="mt-5 min-h-44 rounded-lg bg-gradient-to-br from-blue-100 via-white to-teal-100 p-5 dark:from-blue-950 dark:via-slate-900 dark:to-teal-950">
        ...
        <section id="features" className="mx-auto max-w-7xl px-4 py-10 sm:px-5 sm:py-12"><h2 className="text-2xl font-bold">{t("mainFeatures")}</h2><div className="responsive-grid mt-5">{features.map((feature) => <Card key={feature}><b className="break-words">{feature}</b><p className="mt-2 text-sm text-slate-500">{language === "ar" ? "مصمم بتدفقات واقعية وحالة محفوظة وتجاوب كامل وتحكم سهل الوصول." : "Built with realistic workflows, saved demo state, responsive layout, and accessible controls."}</p></Card>)}</div></section>
        <section id="benefits" className="mx-auto max-w-7xl px-4 py-10 sm:px-5 sm:py-12"><div className="responsive-grid">{["Students stay organized without feeling overwhelmed.", "Teachers manage lessons, grading, quizzes, and attendance in one place.", "Parents see progress, attendance, and school updates clearly."].map((text, index) => <Card key={text}><Badge tone={index === 1 ? "green" : "blue"}>{["Students", "Teachers", "Parents"][index]}</Badge><p className="mt-4 text-slate-600 dark:text-slate-300">{text}</p></Card>)}</div></section>
      </main>
...
export function LoginPage({ setUser, user }) {
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-slate-50 p-3 dark:bg-slate-950 sm:p-4" dir={dir}>
      <div className="grid w-full max-w-5xl overflow-hidden rounded-lg bg-white shadow-xl dark:bg-slate-900 lg:grid-cols-2">
        <div className="bg-gradient-to-br from-blue-700 to-teal-600 p-8 text-white">
          ...
          <h1 className="mt-8 break-words text-3xl font-black sm:text-4xl">{t("welcome")}</h1>
        ...
        <form onSubmit={submit} className="p-5 sm:p-8">
          ...
          <div className="mt-6 space-y-4">
            ...
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200"><span className="mb-1 block">{t("password")}</span><div className="flex rounded-lg border border-slate-300 dark:border-slate-700"><input className="min-h-11 min-w-0 flex-1 rounded-l-lg bg-white px-3 py-2 dark:bg-slate-950" type={show ? "text" : "password"} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /><button aria-label="Toggle password visibility" type="button" className="min-h-11 px-3" onClick={() => setShow(!show)}>{show ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></label>
